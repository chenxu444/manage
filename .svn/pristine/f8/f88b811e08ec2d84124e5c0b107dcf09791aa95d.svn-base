...
export default {
    ...
    methods: {
        ...
        // 跳转首页
        goDetails(item){
            this.goUrl('index', '', 'storeId=' + item.storeId + '&consignorId=' + item.consignorId + '&url=' + encodeURIComponent(this.baseUrl + (item.storePageId ? item.storePageId + '.html' : 'miniIndex.html')));
        },
        goBack() {
            history.go(-1);
        },
