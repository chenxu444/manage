<template>
    <view class="askInfoPage" >
        <!-- #ifndef MP-WEIXIN -->
        <uni-nav-hfbar fixed status-bar left-icon="back" title="处方问诊"></uni-nav-hfbar>
        <!-- #endif -->
        ...
        <view class="main-content bg_white" :style="isiphoneBt ? toggle == 2 ? 'padding-bottom: 260rpx' : 'padding-bottom: 160rpx' : toggle == 2 ? 'padding-bottom: 210rpx' : ''">
            ...
            <view class="visit-wrap">
                <view class="mod-disease" v-show="toggle == 2">
                    <view class="tit">
                        <span>上传复诊凭证</span>
                        <span class="remark">（用于辅助医生诊断；购买胰岛素类药品必传）</span>
                    </view>
                      <view>
                        <span class="remark">有效复诊凭证包括出入院报告/处方单/出院小结/检验报告单等，电子处方暂不能作为线上复诊凭证</span>
                    </view>
                    <view class="records-add clearfix uni-flex">
                        <view class="historyPhotos" v-if="picInfos.length>0">
                            <view class="historyPhotosList clearfix">
                                <view class="img-box records-img" :class="hi.choose?'border_common':''"  v-for="(hi, index) in picInfos" v-if="picInfos.length > 0" @click="cancelImg(index)">
                                    <span>
                                        <image mode="aspectFit" class="askPhotoItem" :src="hi.recipeUrl" />
                                        <b class="iconfont icon-selectImg font_color_common" v-show="hi.choose"></b>
                                    </span>
                                </view>
                            </view>
                        </view>
                ...
                <view class="mod-records" v-show="toggle == 1">
                    <view class="tit">
                        <span>上传医院处方单</span>
                        <span class="remark">（用于药师审核发药）</span>
                    </view>
                    <view class="records-add clearfix">
                        <view class="records-ask-icon img-add-box font_color_common border_common bg_common_light" @click="choiceImage">
                            <i class="iconfont icon-camera"></i>
                            <span>拍照上传<br>处方图片</span>
                        </view>
                    </view>
                ...
                <view class="mod-records" v-show="toggle == 2">
                    ...
                    <div class="pmh_choose uni-flex">
                        ...
                        <div class="pmh_item" @click="addDisease">+</div>
                    </div>
                </view>
            </view>
            <!-- 复诊开方内容结束 -->
        ...
        <view class="btn_wrap" :style="isiphoneBt ? 'padding-bottom: 60rpx' : ''">
            <button class="font_color_white bg_common" @click="saveAskInfo">{{toggle == 1 ? '保存' : '保存并同意就诊'}}</button>
        </view>
        ...
        <uni-popup ref="popup">
            <view class="dialog">
                <view class="dialogTitle">确诊疾病</view>
                <view class="dialogTxt">请输入确诊疾病内容</view>
...
export default {
    ...
    onLoad(option) {
        ...
        this.toggle = beforePage.$vm.recipeToggle ? beforePage.$vm.recipeToggle : '2',
        ...
        this.picInfos = beforePage.$vm.drugerImgArr
        this.pmhArr = beforePage.$vm.pmhArr;
        this.choiceMap = beforePage.$vm.choiceMap;
        this.showRecipe = beforePage.$vm.showRecipe;
        this.storeId = option.storeId;
        if(this.druguser != ''){
            this.selectUserId = this.druguser.userId;
            this.pastHistory = this.druguser.diseases;
        }
        if(this.toggle == 2 && this.showRecipe){
            this.isAgree = true;
        }
        this.getDruger();
        if(this.toggle == 1){
    ...
    methods: {
        ...
        getDruger(){
            // 初始化用药人
            this.doRequest("/usercenter/druguser/list", {storeid: this.storeId}, false).then((res) => {
                for(let i = 0; i < res.length; i++){
                    ...
                    if(this.selectUserId){
                        ...
                        if(res.length == i + 1){
                            ...
                            if(this.toggle == 2 && this.pmhArr.length == 0){
                                this.initDisease();
                            }
                            break;
                        }
        ...
        selectUser(index) {
            ...
            if(this.druguser.liverType === null || this.druguser.liverType === '' || this.druguser.liverType === 'null'){
        ...
        toggleNav(type){
            this.toggle = type;
            if(type == 2 && this.pmhArr.length == 0){
                this.initDisease();
            }else if(type == 1 && this.historyImage.length == 0){
                this.initAllPhotos();
            }
        },
        initDisease(){
            ...
            this.doRequest("/recipecenter/lianou/diseaselabels", obj, false).then((res) => {
                this.pmhArr = res ? res : [];
                this.$nextTick(() => {
                    uni.hideLoading();
                })
        ...
        dialogButton(boolean) {
            if (boolean) {
                if(this.changeTxt.length > 8){
                    uni.showToast({
                        title: "抱歉！疾病名称长度必须小于8个字符",
                        icon: "none",
                    })
                    return;
                }
                let obj = {
                    diseaseName: this.changeTxt,
                    iCDCode: new Date().getTime() + '',
                }
                this.pmhArr.push(obj);
                this.choiceMap[obj.iCDCode] = obj;
                this.iCDCodeArr.push(obj.iCDCode);
            }
            this.changeTxt = '';
        ...
        choiceImage(){
            // 调用本地相册或相机
            if(this.picInfos.length>=3 && this.toggle == 2){
                uni.showToast({
                    title:'最多不能超过3张图片！',
                    icon:'none'
                })
                ...
                success: res => {
                    this.pathToBase64(res.tempFilePaths[0]).then(path => {
                        ...
                        uni.request({
                            ...
                            success: data => {
                                let recipeUrl = this.base + '/attach/download?attachId=' + data.data
                                let imgObj = {
                                    recipeUrl,
                                    recipeId: '',
                                }
        ...
        cancelImg(index){
            this.picInfos[index].choose = !this.picInfos[index].choose
        },
        saveAskInfo(){
            // 保存问诊信息
            if(!this.druguser){
            ...
            if(this.toggle == 1){
                ...
                let historyImageArr = []
                this.picInfos.forEach(item=>{
                    if(item.choose){
                        historyImageArr.push(item.recipeUrl)
                    }
                })
                beforePage.$vm.diseaseInfos = diseaseInfos;
                beforePage.$vm.historyImageArr = historyImageArr;
                beforePage.$vm.drugerImgArr = this.picInfos
                beforePage.$vm.druguser = this.druguser;
                beforePage.$vm.showRecipe = true;
                beforePage.$vm.recipeToggle = this.toggle;
                beforePage.$vm.iCDCodeArr = this.iCDCodeArr;
                beforePage.$vm.choiceMap = this.choiceMap;
                beforePage.$vm.pmhArr = this.pmhArr;
                beforePage.$vm.resetOrderFunc('0');
                this.$nextTick(() => {
                    uni.navigateBack({
                        delta: 1
                    });
