<template>
    <view class="orderPayPage">
        <!-- #ifndef MP-WEIXIN -->
        <uni-nav-hfbar fixed status-bar :border="false" left-icon="back" title="订单支付"></uni-nav-hfbar>
        <!-- #endif -->
...
export default {
    ...
    data() {
        return {
            ...
            totalpromoAmount: 0,
            appid: '',
            openid: this.getStor("__openid__"),
        ...
        orderPay(){
            ...
            this.ebigRequest(this.memberId != ''?'/order/showOrderPayByGuest':'/order/showOrderPay', this.memberId != ''?{'orderNos': this.orderNos}:{'orderNos': this.orderNos,'buyMemberId': this.memberId}, false, '', '', true, true).then((res) => {
                if(res.success == false){
                    ...
                    params.appId = uni.getAccountInfoSync().miniProgram.appId;
                    // #endif

                    // #ifdef H5
                    params.payType = 'icbcjft';
                    params.clientType = 'H5';
                    params.b2cPayType = '3';
                    params.orderNos = this.orderNos;
                    params.openId = this.openid;
                    params.jumpurl = this.baseUrl + 'order/pages/orderSuccess/orderSuccess?orderNo=' + this.orderNos;
                    // #endif
                    ...
                    this.ebigRequest('/epaycenter/ebig_paysrv/B2cOrder/prepare', params, false).then((data) => {
                        ...
                        // #ifdef H5
                        let payform = document.getElementsByTagName('payform');
                        if (payform.length) {
                            document.body.removeChild(payform[0]);
                        }
                        const div = document.createElement('payform');
                        div.innerHTML = data; // res.data就是返回给你的form
