<template>
    <view id="page" style="width: 100%;">
        <!-- #ifndef MP-WEIXIN -->
        <uni-nav-hfbar fixed status-bar left-icon="back" title="推广商品列表"></uni-nav-hfbar>
        <!-- #endif -->
        <view class="main-wrap">
            ...
            <view class="tab-wrap uni-flex font_color_white bg_white">
                ...
                <view class="tab-item uni-flex-1 border_common tc" :class="activeNav == 'commission' ? 'on font_color_common' : 'font_color_666'" @tap="tabClick('commission');">积分</view>
                            ...
                            <view class="goods-info uni-flex-1">
                                ...
                                <view class="price-wrap uni-flex">
                                    ...
                                    <view class="can-get-price tr uni-flex-1">
                                        <span class="price-3">{{ goods.commission ? toFixedFunc(goods.commission) : '0.00' }}</span>
                                        <span class="price-3-des font_color_999">积分</span>
                                    </view>
                                    <!-- #endif -->
                                </view>
                                <!-- #ifdef MP-WEIXIN -->
...
export default {
    ...
    data() {
        return {
            activeNav: 'normal', //normal：综合 commission：积分 distributionDate：时间 salNum：销量 price：价格
            keyword: '', //搜索关键字
            pageNo: 1,
            pageSize: 10,
            list: [],
    ...
    onLoad(option) {
        ...
        if (this.getStor("_isDistributorId_") != null) {
            this.nowShareUrl = this.nowShareUrl + (this.getStor("_isDistributorId_") != null ? "?shareDistributorId=" + this.getStor("_isDistributorId_") : '');
        }
    },
    onShow() {
    ...
    methods: {
        openDetail(goods){
            // 跳转商品详情
            this.goUrl('detail', '', 'skuSalNo=' + goods.skuSalNo + '&storeId=' + goods.storeDTO.storeId + '&consignorId=' + goods.consignorId + '&isDistributGoods=true');
        },
        ...
        shareClick: function(goods) { //点击商品分享
            ...
            this.shareUrl = this.shareUrl + '?skuSalNo=' + goods.skuSalNo + '&storeId=' + goods.storeDTO.storeId + '&consignorId=' + goods.consignorId + '&shareDistributorId=' + this.getStor('isDistributorId') + '&distributionId=' + goods.distributionId;
        ...
        getList: function(type) { //获取推广商品列表数据
            ...
            obj.sortFiled = this.activeNav; //normal：综合 commission：积分 distributionDate：时间 salNum：销量 price：价格
            obj.sortType = 'desc'; //asc desc
            obj.queryWord = this.keyword; //搜索关键字
            obj.pageNo = this.pageNo;
            obj.pageSize = this.pageSize;
            obj.consignorId = this.getStor('_defaultConsignorId_');
