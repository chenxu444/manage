...
export default {
  ...
  methods: {
    ...
    getCode() {
      ...
      this.doRequest(
        "/usercenter/captcha/"+this.mobile,
        {},
        true,
        'get'
      ).then((res) => {
    ...
    doLogin(e) {
      ...
      this.doRequest("/usercenter/member/login/mobile",
          {
            mobile: mobile,
            smsCode: smsCode,
            ...
            distributorId:this.getStor('shareDistributorId')?this.getStor('shareDistributorId'):''
          },
        true
      ).then((res) => {
        if (res) {
          ...
          //判断用户是否需要实名
          this.doRequest("/usercenter/member/requireRealName", {}, true, 'get').then((data) => {
