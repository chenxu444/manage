    ...
    <view id="page" style="width: 100%;height: 100%;">
        <view class="wrap">
                    ...
                    <view class="mod-part part-1">
                        ...
                        <view class="col-sha uni-flex font_color_333">
                            <view class="col uni-flex-1 tc" @click="collectFunc(goodsInfo.skuSalNo, storeInfo.storeId, goodsInfo.collectFlag)">
                                <span class="iconfont icon-collect" :class="goodsInfo.collectFlag ? 'font_color_common' : 'font_color_999'"></span> {{ goodsInfo.collectFlag ? '已收藏' : '收藏' }}
                            </view>
                        ...
                        <view class="goodsInfo uni-flex">
                            <view class="uni-flex-1">
                                ...
                                <view class="price uni-flex font_color_333" v-if="tjList.length == 0">
                                    <view class="p-l" v-if="redeemId">
                                        <span>
                                                    {{ needIntegral }}<span class="p-t">积分</span>
                                        </span>
                                    </view>
                                    <view class="p-l" v-else-if="goodsInfo.sellPrice">
                                        <span>
                                                    <span class="p-t">￥</span>{{ toFixedFunc(goodsInfo.sellPrice ? goodsInfo.sellPrice.sellPrice : 0) }}
                                        </span>
                                        <span class="p-m font_color_999" v-if="goodsInfo.sellPrice && goodsInfo.sellPrice.marketPrice && goodsInfo.sellPrice.marketPrice * 1 > goodsInfo.sellPrice.sellPrice * 1">￥{{ toFixedFunc(goodsInfo.sellPrice ? goodsInfo.sellPrice.marketPrice : 0) }}</span>
                                    </view>
                                    <view class="p-l-n" v-else>暂无定价</view>
                                    <view class="p-r uni-flex-1 tr" v-if="!redeemId">库存：{{ goodsInfo.stockQty > 0 ? goodsInfo.stockQty : '暂无' }}</view>
                                </view>
                                <view class="fanc">厂家：{{ goodsInfo.factoryName ? goodsInfo.factoryName : '--' }}</view>
                            </view>
                            <view class="get bg_common_light font_color_common" v-if="isDistributGoods" v-cloak>
                                <p class="font_color_333">预估收益</p>
                                <span class="get-des"></span>{{ commission }}<i style="display: inline-block; font-size: 24rpx;">积分</i>
                            </view>
                        </view>
                        <!-- 商品资料 -- end -->
                    </view>
                        ...
                        <view class="storeInfo">
                            ...
                            <view class="tip-wrap uni-flex font_color_000" @click="popName = 'tip';">
                                <view class="tipItem uni-flex-1 tc">
                                    <i class="iconfont icon-selected font_color_common"></i> 正品保障
                                </view>
                                <view class="tipItem uni-flex-1 tc">
                                    <i class="iconfont icon-selected font_color_common"></i> 便捷支付
                                </view>
                                <view class="tipItem uni-flex-1 tc">
                                    <i class="iconfont icon-selected font_color_common"></i> 及时配送
                                </view>
                                <view class="tipItem uni-flex-1 tc">
                                    <i class="iconfont icon-selected font_color_common"></i> 专业药师
                                </view>
                            </view>
                        </view>
                        <!-- 门店信息 -- end -->
            ...
            <view class="bTab-wrap uni-flex tc bg_white">
                ...
                <view class="m-box bTab-cart" @click="goUrl('cart')">
                ...
                <view class="uni-flex-1 bTab-btn-nowBuy font_color_white" v-if="redeemId" @click="openDiaSpc()">
                    立即兑换
                </view>
                <view class="uni-flex uni-flex-1" v-else>
                    <view class="uni-flex-1 bTab-btn-addCart font_color_white" :style="goodsInfo.sellPrice && goodsInfo.sellPrice != undefined && goodsInfo.sellPrice != null && goodsInfo.sellPrice != '' && goodsInfo.status != -1 && goodsInfo.marketFlag != 0 ? 'background: #f7bf57;' : 'color: #999; background: #eee;'"
                        v-if="goodsInfo.status != 2" @click="joinCart()">加入清单</view>
                    <view class="uni-flex-1 bTab-btn-nowBuy font_color_white" :style="goodsInfo.sellPrice && goodsInfo.sellPrice != undefined && goodsInfo.sellPrice != null && goodsInfo.sellPrice != '' && goodsInfo.status != -1 && goodsInfo.marketFlag != 0 ? 'background: #e35c5c;' : 'color: #999; background: #eee;'"
                        v-if="goodsInfo.status != 2" @click="openDiaSpc()">{{ goodsInfo.recipeFlag == 2 ? '提交需求' : '立即购买' }}</view>
            ...
            <view class="pop-wrap" :class="popName != 'none' ? 'on' : ''">
                <view class="mod-dia">
                            ...
                            <scroll-view scroll-y="true" show-scrollbar="false" style="height: 100%;">
                                <view class="pop-coupon-wrap">
                                                ...
                                                <view class="coupon-con-wrap uni-flex">
                                                    ...
                                                    <view class="couponCon-info uni-flex-1">
                                                        <view class="info-head font_color_666">
                                                            <span class="coupon-type bg_common font_color_white">{{ coupontypeFunc(coup.couponContentType) }}</span> {{ coup.couponCfgName }}
                                                        </view>
                                    ...
                                    <view class="mod-coupon mod-coupon-getyet" v-if="gotCoupons.length > 0">
                                        ...
                                        <view class="pop-coupon-list">
                                            <view class="pop-coupon-item border_common" v-for="(coup,index) in gotCoupons">
                                                <view class="coupon-con-wrap uni-flex">
                                                    <view class="couponCon-main bg_common font_color_white">
                                                        <span class="main-type" v-if="coup.couponContentType == 1 || coup.couponContentType == 7">￥</span>
                                                        <span class="main-con">{{ couponTypeFilter(coup.couponContent, coup.couponContentType) }}</span>
                                    ...
                                    <view class="mod-coupon mod-coupon-cannotuse" v-if="cannotGetCoupons.length > 0">
                                        ...
                                        <view class="pop-coupon-list">
                                            <view class="pop-coupon-item border_common" v-for="(coup,index) in cannotGetCoupons">
                                                <view class="coupon-con-wrap uni-flex">
                                                    <view class="couponCon-main bg_common font_color_white">
                                                        <span class="main-type" v-if="coup.couponContentType == 1 || coup.couponContentType == 7">￥</span>
                                                        <span class="main-con">{{ couponTypeFilter(coup.couponContent, coup.couponContentType) }}</span>
                    ...
                    <view class="pop pop-spec" :class="popName == 'spec' ? 'fade-in-up on' : 'fade-out-down'" id="pop-spec">
                        ...
                        <view class="pop-btn-wrap uni-flex" v-if="!redeemId">
                        ...
                        <view class="pop-btn-wrap" v-else>
                            <view class="pop-btnItem pop-btn-buyNow" @click="addRedeemFunc">立即兑换</view>
                        </view>
                    </view>
                    <!-- 加入清单、立即购买弹窗 -- end -->
                    <!-- 基本信息弹窗 -- start -->
        ...
        <!-- 生成海报 -->
        <!-- #ifdef H5 -->
        <view class="sharePic" id="sharePic">
            <view class="sharePicCont bg_white" id="sharePicCont" :style="'width: ' + swiperHeight + 'px;'">
                <view class="picInfo">
                    <view class="picInfoImg">
                        <img :src="goodImgArr.length ? goodImgArr[0] + '?v=isok' : ''" crossorigin="anonymous" :style="actType != '' ? 'width: 48%;' : ''">
                    </view>
                </view>
                <view class="storeInfo">
                    <view class="storeName flex-1 font_color_333">{{goodsInfo.goodsName}}</view>
                    <view class="price font_color_red">
                        <span v-if="goodsInfo.sellPrice">￥{{ toFixedFunc(goodsInfo.sellPrice ? goodsInfo.sellPrice.sellPrice : 0) }}</span>
                    </view>
                </view>
                <view class="goodsWrap">
                    <view class="code" id="qrcode"></view>
                    <view class="pgoodsInfo">
                        <p>{{canvasAttr.codeName}}</p>
                        <p class="tips">{{canvasAttr.tips}}</p>
                    </view>
                </view>
            </view>
        </view>
...
<script>
    ...
    export default {
        ...
        onLoad(option) {
            let myDistributorId = this.getStor("isDistributorId");
            
            if(option.activitysaleId){
                this.activitysaleId = option.activitysaleId
            }
            if (option.redeemId) {
                this.redeemId = option.redeemId
                this.getInteDetail(option.redeemId)
                this.shareUrl += '?redeemId=' + option.redeemId
            }
            if (option.skuSalNo) {
                ...
                this.isDistributGoods = option.isDistributGoods && option.isDistributGoods == 'true' ? true : false;
                this.distributorId = option.shareDistributorId && option.shareDistributorId != 'null' ? option.shareDistributorId : this.getStor('shareDistributorId') != null ? this.getStor('shareDistributorId') : '';
                if (this.isDistributGoods) {
                ...
                this.doRequest("/wechatcenter/qrcode/v1/scene/" + option.scene, {}, false, 'get').then((data) => {
                    ...
                    for (var i = 0; i < sceneArr.length; i++) {
                        ...
                        if (item.indexOf('shareDistributorId') != -1) {
                            ...
                            this.distributorId = this.getStor('shareDistributorId') != null ? this.getStor('shareDistributorId') : '';
                        }
                    }
                    this.shareUrl = this.shareUrl + '?skuSalNo=' + this.skuSalNo + '&storeId=' + this.storeId;
                    let sysInfo = uni.getSystemInfoSync();
            ...
            this.$nextTick(() => {
                if (myDistributorId != null && myDistributorId != '') {
                    this.shareUrl = this.shareUrl + '&shareDistributorId=' + myDistributorId;
                }
            })
        },
        onShareAppMessage() { //分享到微信好友
            ...
            return {
                ...
                content: this.storeInfo.storeName,
        ...
        methods: {
            creatBill() { //生成海报
                // #ifdef MP-WEIXIN
                this.canvasAttr.posterCodeUrl = this.wechat_base + "/wechatcenter/qrcode/v1/" +
                    this.getStor("_appid_") + "/getUnlimitedCode?scene=" +
                    encodeURIComponent(this.shareUrl.substring(21, this.shareUrl.length)) + '&width=280&page=pages/detail/detail';
                this.$nextTick(() => {
                ...
                setTimeout(async() => {
                    await this.couponQrCode();
                    let share = document.querySelector("#sharePicCont");
                    html2canvas(share, {
                        x: 0, // 绘制的dom元素相对于视口的位置
                        y: 0,
                        scrollY: 0,
            ...
            checkShareAct() { //判断是否需要记录分享活动
                ...
                if (promotionId != '' && promotionId != null && promotionId != undefined) {
                    ...
                    var shareGoodsid = this.getStor('_shareGoodsid_');
                    if (shareGoodsid == this.skuSalNo) {
                        promotionId = this.getStor('_sharePromotionId_');
                        sharerId = this.getStor('_sharerId_');
                        this.takeShareAct(promotionId, sharerId);
                    }
                }
            },
            takeShareAct(promotionId, sharerId) { //记录是否是分享进来的
                var obj = {};
                ...
                this.ebigRequest("/share/createShareRecord", obj, false).then((data) => {
                    // console.log(data) 
                    if (data) {
                        this.removeStor('_sharePromotionId_');
                        this.removeStor('_sharerId_');
                        this.removeStor('_shareGoodsid_');
                    }
                }).catch(err => {})
            },
            copyShareFunc() { //分享复制链接
            ...
            getShareMan() { //获取分享人信息
                ...
                this.doRequest("/usercenter/member/find", obj, false, '', '', true).then((data) => {
                    // console.log(data)
                    if (data) {
            ...
            getShareAct(consignorId) { //获取分享活动
                var obj = {};
                ...
                this.ebigRequest("/share/getSharePromotioin", obj, false).then((data) => {
                    ...
                    if (data) {
                        ...
                        this.setStor('_shareUrl_', this.shareUrl);
                    }
                    // #ifdef H5
                    this.$nextTick(() => {
                        this.sharePageFunc(this.goodsInfo.brandName + ' ' + this.goodsInfo.goodsName + ' ' + this.goodsInfo.specification, this.storeInfo.storeName, this.shareUrl, this.goodImgArr[0]);
                    })
                    // #endif
                }).catch(err => {
                    // #ifdef H5
            ...
            shareHtml(contentDTOS) { //组装分享内容html
            ...
            getDtlData(id) { // 获取商品详情
                ...
                this.doRequest(this.isDistributGoods ? "/distributorcenter/distribution/product" : "/web-omall/product/details", obj, true, this.isDistributGoods ? 'get' : 'post').then((data) => {
                    // console.log(data)
                    if (data) {
                        this.goodsInfo = data;
                        ...
                        if (images.length == 0) {
                            images.push('../../static/image/default/no_img.jpg')
                        }
            ...
            getInteDetail(id) {
                let obj = {};
                obj.redeemId = id;
                this.ebigRequest("/redeemProduct/detail", obj, true).then((res) => {
                    // console.log(data)
                    if (res) {
                        var data = res.productDTO
                        this.needIntegral = res.needIntegral;
                        this.goodsInfo = data;
                        this.storeInfo = data.storeDTO;
                        // 商品图片组装 -- start
                        var images = [];
            ...
            getActData(storeId, skuId) { //获取商品活动数据
                let obj = {};
                obj.storeid = storeId;
                obj.skuid = skuId;
                this.ebigRequest("/promotion/getPromotionsByProducts", obj, false).then((data) => {
                    ...
                    this.$nextTick(() => {
                        ...
                        // #ifdef H5
                        // let getUrl = {};
                        // getUrl.targetUrl = this.getAuthorUrl(encodeURIComponent(this.shareUrl));
                        this.sharePageFunc(this.goodsInfo.brandName + ' ' + this.goodsInfo.goodsName + ' ' + this.goodsInfo.specification, this.storeInfo.storeName, this.shareUrl, this.goodImgArr[0]);
                        // console.log(this.wechat_base+ '+' + getUrl);
                        // this.ebigRequest("/url/create", getUrl, true, 'post', '', true, true).then((data) => {
                        //     this.nowUrl = this.wechat_base + data;
                        // })
            ...
            getCouponData(storeId, skuId) { //获取商品优惠券数据
                let obj = {};
                obj.pageNo = '1';
                obj.pageSize = '999';
                obj.skuId = skuId;
            ...
            getYouList(storeId, skuId) { // 获取猜你喜欢数据列表
                let obj = {};
                obj.pageNo = 1;
                obj.pageSize = 8;
                obj.storeSkuIds = [{
            ...
            initCartFunc() { //获取清单列表数量
                this.ebigRequest("/shopping-cart/getCartItemCount", {}, false, '', '', true, true).then((data) => {
                    // console.log(data)
                    if (data) {
                        this.cartGoodsQty = data.data;
            ...
            joinCart() { // 加入清单
                ...
                } else if (this.getStor('isDistributorId') && this.getStor('isDistributorId') != 'null') {
                    obj.distributorId = this.getStor('isDistributorId');
                }
                if(this.activitysaleId){
                    obj.activitysaleId = this.activitysaleId
                }
                this.ebigRequest("/shopping-cart/add", obj, true).then((data) => {
                    // console.log(data)
                    if (data) {
                        this.cartGoodsQty = data.totalItemCount;
                        this.setStor('totalItemCount', data.totalItemCount);
                        uni.showToast({
                            title: "已加入清单!",
                            icon: "none",
                        });
            ...
            addRedeemFunc() { // 立即兑换
                let obj = {};
                obj.redeemId = this.redeemId;
                obj.quantity = this.qtyvalue;
                this.ebigRequest("/redeemOrder/create", obj, true).then((data) => {
                    // console.log(data)
                    if (data) {
                        this.closePop();
                        this.goUrl('orderConfirm', 'order', 'orderNo=' + data + '&isRedeem=true');
                    }
                }).catch(err => {})
            },
            playPhone(mobile) { //拨打电话
                uni.makePhoneCall({
                    phoneNumber: mobile
                });
            },
            openDiaSpc() { //点击立即购买、提交需求判断是否打开spc弹窗
                if (this.goodsInfo.status == '-1') {
                    uni.showToast({
                        title: "抱歉,该商品不能购买!",
                        icon: "none",
            ...
            groupAddCart() { //组合商品加入购物车
                let qty = 1;
                if (this.groupDtlInfo.leastCount != undefined && this.groupDtlInfo.leastCount != null && this.groupDtlInfo.leastCount != '') {
                    qty = this.groupDtlInfo.leastCount;
                }
                let obj = {};
                obj.storeId = this.groupInfo.storeId;
                obj.bundleId = this.groupInfo.id;
                obj.quanlity = qty;
                this.ebigRequest("/shopping-cart/addBundleItem", obj, true).then((data) => {
                    // console.log(data)
                    if (data) {
                        this.cartGoodsQty = data.totalItemCount;
                        this.setStor('totalItemCount', data.totalItemCount);
            ...
            collectFunc(skuSalNo, storeId, collectFlag) { //收藏商品
                let obj = {};
            ...
            getCoupon(couponCfgId, index) { // 领取优惠券
                let obj = {};
            ...
            couponTypeFilter(value, couponcontenttype) { //优惠券内容过滤 
            ...
            couponContFilter(value) { // 优惠券单位过滤
                if (value == 2) {
                    return '折';
                } else if (value == 4) {
                    return '倍';
            ...
            couponRuleFilter(value, res) { // 优惠券规则过滤
                if (value) {
            ...
            recipeFlagFilter(value) { // 商品是否为处方药
                if (value == 1) {
            ...
            goodsValFilter(type, value) { //商品信息过滤
            ...
            changeImgSize(value, num) { // 原图转成其他尺寸图
                if (!value) {
            ...
            countDown(startTime, endDate) { //倒计时
            ...
            DateMinus(date1, date2) { //判断时间是否已过
                var sdate = new Date(date1);
                var now = new Date(date2);
                var days = now.getTime() - sdate.getTime();
                return days;
            },
            getDtlTop() { //获取详情距离顶部高度
            ...
            showMoreCoup(type, index) { //查看优惠券详情
        ...
        .pop-wrap {
            ...
            .mod-dia {
                ...
                .pop {
                    ...
                    &.pop-baseInfo .pop-btn-close,
                    &.pop-explain .pop-btn-close,
                    &.pop-tip .pop-btn-close,
