<template>
    <view id="page" style="width: 100%;height: 100%;">
        <!-- #ifndef MP-WEIXIN -->
        <uni-nav-hfbar fixed status-bar left-icon="back" title="成为推广员"></uni-nav-hfbar>
        <!-- #endif -->
        <view class="shareInfo uni-flex" v-if="code != ''">
            <image class="shareImg" :src="headImgUrl" v-if="headImgUrl != ''" />
            <view class="shareTips uni-flex-1"><span class="font_color_common fontWeight">{{idName}}</span>邀请你成为推广员</view>
        </view>
        <view class="main-wrap">
            <div class="content_txt">线上推广中心</div>
            <div class="content_txt2">成为推广员</div>
            <!-- content -- start -->
            <view class="content-wrap" :style="isiphoneBt ? 'padding-bottom: 60rpx;' : ''">
                ...
                <view class="btn-become tc bg_common font_color_white" @click="becomeClick" :style="isiphoneBt ? 'bottom: 60rpx;' : ''">成为推广员</view>
            </view>
            <!-- content -- end -->
        </view>
    </view>
...
export default {
    ...
    methods: {
        checkStatus(){
            this.doRequest('/distributorcenter/distributor/check_status', {consignorId: this.getStor('_defaultConsignorId_')}, true, 'get').then(res => {
                if(res == 'APPROVED' || res == 'DISABLED'){
                    this.replaceUrl("promotionCenter", "distribution");
                } else if(res == 'COMMIT' || res == 'REFUSED'){
                    this.replaceUrl("applyInfo", "distribution", 'status=check' + (this.code ? '&code=' + this.code : ''));
                }
            });
        },
        initMan(){
