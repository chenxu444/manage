...
export default {
  ...
  methods: {
    ...
    onClick(e, userId) {
      ...
      if (e.content.text == "设为默认") {
        // 设置默认
        this.doRequest("/usercenter/druguser/markDefault", params, true).then(
          (res) => {
              this.getDruger();
          }
        );
        ...
        // 删除
        this.doRequest("/usercenter/druguser/delete", params, true).then((res) => {
          if (res) {
            uni.showToast({
              title: "删除成功",
              icon: "none",
    ...
    getDruger() {
      ...
      this.doRequest("/usercenter/druguser/list", params, true).then((res) => {
        this.content = res;
  ...
  onShow() {
    this.removeStor('editDruguser')
  },
  onLoad(options){
    this.storeId =options.storeId || this.getStor('_defaultStoreId_');
    this.getDruger();
  }
};
</script>
