  ...
  methods: {
    ...
    posterShow() {
      ...
      this.attrs = {
        ...
        minTitle: this.attrs.minTitle,
        minTitleFontSize: this.attrs.minTitleFontSize * this.systemScale,
        minTitleLineHeight: this.attrs.minTitleLineHeight * this.systemScale,
     ...
     * @param {number} minTitle 海报的小title
     * @param {number} minTitleFontSize 小title字体大小
     * @param {number} minTitleLineHeight 小标题文本的行高大小
    ...
    async creatPoster(canvasAttr) {
      ...
      if (this.posterBgFlag && canvasAttr.posterBgUrl) {
      ...
      let textMY = this.creatMinTitle(ctx, canvasAttr, textY);
      // 绘制小程序码
      this.creatCode(ctx, canvasAttr, textMY ? textMY : textY);
      // // 小程序的名称
      this.creatCodeText(
        ctx,
        canvasAttr,
        ...
        uni.getImageInfo({
          ...
          success(res) {
            ...
            _this.roundRect(
              ...
              canvasAttr.marginTB + canvasAttr.innerLR,
    ...
    creatMinTitle(ctx, canvasAttr, textY) {
      ctx.restore(); //恢复之前保存的绘图上下文
      ctx.save();
      ctx.setFontSize(canvasAttr.minTitleFontSize);
      ctx.setFillStyle("#E35C5C"); //文字颜色：默认黑色
      let textMY = this.drawText(
        ctx,
        canvasAttr.minTitle,
        canvasAttr.marginLR + canvasAttr.innerLR,
        textY + canvasAttr.titleLineHeight + 4,
        this.system.w - 2 * canvasAttr.marginLR - 2 * canvasAttr.innerLR,
        canvasAttr.minTitleLineHeight
      );
      ctx.draw(true);
      return textMY;
    },
    ...
    creatVerticalTitle(ctx, canvasAttr) {
      ...
      canvasAttr.title.forEach((element, i) => {
        this.drawText(
          ctx,
          element,
          canvasAttr.marginTB + canvasAttr.desTextML + i * 20,
          canvasAttr.marginLR + canvasAttr.desTextMT,
          15,
          canvasAttr.titleLineHeight
        );
      });
    ...
    creatCodeText(
      ...
      ctx.fillText(
        text,
        (this.system.w - testWidth) / 2,
        textY +
          canvasAttr.codeMT +
          this.system.w * canvasAttr.codeWidth +
          canvasAttr.codeNameMT +
          10
      );
      // 小程序的名称end
    },
    ...
    creatSimpleCode(ctx, canvasAttr) {
      ...
      wx.getImageInfo({
        ...
        success(res) {
          ...
          _this.roundRect(
            ctx,
            canvasAttr.marginLR + canvasAttr.codeML,
            canvasAttr.marginTB + canvasAttr.codeMT,
            _this.system.w * canvasAttr.codeWidth,
            _this.system.w * canvasAttr.codeWidth * canvasAttr.codeRatio,
            _this.system.w *
              canvasAttr.codeWidth *
