	...
	<view class="w-picker">
		...
		<view class="w-picker-cnt" :class="{'show':showPicker}">
			<view class="w-picker-hd" @touchmove.stop.prevent catchtouchmove="true">
			  <view class="w-picker-btn" @tap="pickerCancel" style="border: 1px solid #DDDDDD;">取消</view>
			  <view class="w-picker-btn bg_common font_color_white" :style="{'color':themeColor}" @tap="pickerConfirm">确定</view>
			</view>
	...
	export default {
		...
		props:{
			mode:{
				type:String,
				validator(mode){
					let modeList=['half','date', 'dateTime', 'dateTime', 'yearMonth','time','region','selector','limit','limitHour','range','linkage'];//过滤无效mode;
