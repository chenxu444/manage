<template>
    <view class="promotionOrderList">
        ...
        <!-- #endif -->
        <!-- 顶部nav -->
        <view class="nav  font_color_666">
            <view class=" bg_white uni-flex nav_wrap">
                ...
                <view class="uni-flex-1 chooseTime font_color_999 uni-flex">
                    <span style="font-size:24rpx;" @click="open" :class="isChangeTime?'font_color_common':''">更多条件</span>
                    <span class="iconfont icon-right chooseIcon" @click="open" :class="isChangeTime ? 'changeIcon font_color_common' : 'font_color_999'"></span>
                </view>
            </view>
            <!-- <view class="unNormal uni-flex"><view class="selected2" :class="valid==true?'':'bg_common_light font_color_common'" @click="validOrder">异常订单</view></view> -->
        </view>
        <view class=" time-mod-wrap"  v-show="isChangeTime">
            <view class=" uni-flex item_wrap">
                <view class=" font_color_999">查询时间：</view>
                <view class=" uni-flex-1 font_color_999" @click="openDate('start')" >{{ startDate||'开始日期' }}</view>
                <view class="time-to tc">至</view>
                <view class="time-start uni-flex-1 font_color_999" @click="openDate('end')">{{ endDate||'结束日期' }}</view>
            </view>
            <view class=" uni-flex item_wrap">
                ...
                <view class="totalContent">
                    <view class="orderItem bg_white" @click="toPage('promotionOrderDetail','distributeTradeId='+item.distributeTradeId)" v-for="(item,index) in orderList" :key="index">
                        ...
                        <view class="user uni-flex">
                            ...
                            <view class="orderStatus font_color_999" style="width:300rpx" v-if="isDistributorId != item.distributorId">帮赚推广员：{{item.distributorName}}</view>
                        </view>
                        ...
                        <view class="totalPrice font_color_999">订单总价：￥{{(item.orderDtlDTO.totalActPayAmount?item.orderDtlDTO.totalActPayAmount:0) | toFixed}}</view>
