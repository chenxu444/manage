<template>
  <div class="customer">
      ...
      <div class="customer_list">
          <div class="customer_num">成员 ( {{customerList.length}} ) </div>
          <div class="customer_item bg_white" v-for="(item,index) in customerList" :key="index" @click="toPage('customerInfo','distributorMemberId='+item.distributorMemberId+'&memberId='+item.memberId)">
              <div class="customer_info uni-flex">
                  <img :src="item.headimgUrl?item.headimgUrl:'../../static/image/user/user_noimg.jpg'" class="goods_img">
                  <div class=" customer_name  ">
                        <div>{{item.memberNickname||toPhone(item.memberName)}}</div>
                        <div class="font_color_999 invite_time">邀请时间：{{item.inputdate?dateSub(item.inputdate):'-'}}</div>
                  </div>
                  <div class="help_gain uni-flex-1" >
                      <!-- <div style="padding-bottom: 16rpx;">vip</div> -->
                      <span class="money_title font_color_999">帮赚积分:</span>
                      <span class="money font_color_common"> {{toFixed(item.totalSalaryAmount ? item.totalSalaryAmount*1>9999?'9999+':item.totalSalaryAmount : 0)}}</span>
                  </div>
              </div>
...
export default {
    ...
    methods:{
        ...
        getMycustomer(type){
            ...
            obj.distributorId = this.isDistributorId
