...
export function ebigRequest(url, params, loading, method, txt, showAlert, getAll) {
    return new Promise((resolve, reject) => {
        ...
        if(!params['consignorId'] && method != 'json'){
            params['consignorId'] = getStor('_defaultConsignorId_')
        }
        uni.request({
            ...
            success: res => {
                ...
                if (url == '/login/mobile') {
                    removeStor('_token');
                    setStor("_token", res.data.data != null ? res.data.data.token : '');
                }
                if (!data.success && !showAlert) {
                    if (data.code >= 2000 && data.code <= 2100 && !showAlert) {
                        // #ifdef H5
                        setStor("_returnUrl_", window.location.href);
                        // #endif
                        uni.showModal({
                            title: '温馨提示',
                            content: '登录已失效，请重新登录',
...
export function doRequest(url, params, loading, method, txt, showAlert, getAll) {
    return new Promise((resolve, reject) => {
        let header = {};
        let requestMethod;
        let _token = Vue.prototype.getStor("_token");
        header['authorization'] = _token;
        if(_token != null && _token != 'null' && _token != ''){
            _token = 'authorization=' + _token;
        }
        ...
        uni.request({
            url: (url.indexOf("/wechatcenter") > -1 ? Vue.prototype.wechat_base: (url.indexOf("/web-omall") > -1? Vue.prototype.base.substr(0,Vue.prototype.base.indexOf('/web-omall')) : Vue.prototype.server_base)) + url,//指定使用正式环境微信服务或web服务
            data: params,
            header: header,
            method: requestMethod,
            dataType: 'json',
 ...
 * @param {*} url 
 */
 export function gotoOtherMini(url, appid, item){
    uni.navigateToMiniProgram({
        appId: appid,
        path: url,
        // extraData: item ? item : {},
        // envVersion: item.envversion||'develop',
...
/**
 * @name: 存setStorageSync缓存
 */
export function setStor(key, val) {
    uni.setStorageSync(Vue.prototype.cacheType + key, val);
}

/**
 * @name: 取getStorageSync缓存
 */
export function getStor(key) {
    return uni.getStorageSync(Vue.prototype.cacheType + key);
}

/**
 * @name: 删除removeStorageSync缓存
 */
export function removeStor(key) {
    return uni.removeStorageSync(Vue.prototype.cacheType + key);
}

/**
 * @name: sharePageFunc微信分享H5版本调用方法
 */
export function sharePageFunc(linktitle, linkdesc, linkurl, linkimg) {
    // #ifdef H5
    let nowUrl = linkurl ? linkurl : window.location.href;
    if(nowUrl.indexOf('?shareDistributorId=') > -1){
        let firstUrl = nowUrl.substring(0, nowUrl.indexOf('?') + 1);
        let lastUrl = nowUrl.substring(nowUrl.indexOf('&'), nowUrl.length);
        nowUrl = firstUrl + lastUrl;
    }else if(nowUrl.indexOf('&shareDistributorId=') > -1){
        let firstUrl = nowUrl.substring(0, nowUrl.indexOf('&shareDistributorId='));
        let lastUrl = nowUrl.substring(nowUrl.indexOf('&shareDistributorId=') + 1, nowUrl.length);
        lastUrl = lastUrl.indexOf('&') > -1 ? lastUrl.substring(lastUrl.indexOf('&'), lastUrl.length) : '';
        nowUrl = firstUrl + lastUrl;
    }
    let title = linktitle ? linktitle : document.title;
    let desc = linkdesc ? linkdesc : Vue.prototype.setStor('_storeName_');
    let url = nowUrl + (Vue.prototype.setStor("isDistributorId") && Vue.prototype.setStor("isDistributorId") != 'null' ? (nowUrl.indexOf('?') > -1 ? '&' : '?') + 'shareDistributorId=' + Vue.prototype.setStor("isDistributorId") : '');
    let img = linkimg ? linkimg : Vue.prototype.setStor('_shareLogo_')
    Vue.prototype.sharePlatform(title, desc, url, img);
    // #endif
}