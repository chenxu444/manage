                ...
                <view id="product_list_line" class="prolistLine" v-if="productList.length > 0">
					<view class="pro-item" :key="index" v-for="(product, index) in productList">
						<view class="uni-flex" @click="goUrl('detail', '', 'skuSalNo=' + product.skuSalNo + '&storeId=' + product.storeDTO.storeId + '&consignorId=' + product.consignorId)">
							<img class="goodsImg" v-lazy mode="aspectFit" :src="product.imagePath != null ? product.imagePath : '../../static/image/default/no_img.jpg'" />
							<view class="uni-flex-1">
								<view class="pro-name wrap-2">
									<image v-lazy class="recipeFlag" :src="product.recipeFlag | showrecipeFlag"
										v-if="product.recipeFlag != '' && product.recipeFlag != null && product.recipeFlag != undefined && product.recipeFlag != '3'"></image>{{(product.brandName?product.brandName:'') + ' ' + product.goodsName + ' ' + product.specification}}
								</view>
...
export default {
	data() {
		return {
			...
			couponCfgId: '',
			tagNo: '',
			latitude: this.getStor('nowLat') == null || this.getStor('nowLat') == 'null' ? 0 : this.getStor('nowLat'),
            longitude: this.getStor('nowLng') == null || this.getStor('nowLng') == 'null' ? 0 : this.getStor('nowLng'),
			pageNo: 1,
			pageSize: 10,
			productList: '',
            scrollTop: 0, //初始化滚动高度
            ...
            sortAsc: '', //升/降序
            distributorId: '',
            shareUrl: '/pages/list/list',
		};
    },
    onShow() {
        this.cartQty = this.getStor('totalItemCount') ? this.getStor('totalItemCount') : 0;
    },
    mounted(){
        
    },
	onLoad(options) {
        this.storeId = this.getStor('_storeId_') != null ? this.getStor('_storeId_') : this._storeId_;
        ...
        this.distributorId = options.shareDistributorId ? options.shareDistributorId : this.getStor('shareDistributorId') != null ? this.getStor('shareDistributorId') : '';
        this.setStor('shareDistributorId', this.distributorId);
        if(this.brandId){
            this.shareUrl = this.shareUrl + '?brandId=' + this.brandId;
        }
        if(this.goodsTypeNo){
            this.shareUrl = this.shareUrl + (this.shareUrl.indexOf('?') != -1 ? '&' : '?') + 'goodsTypeNo=' + this.goodsTypeNo;
        }
        if(this.promotionId){
            this.shareUrl = this.shareUrl + (this.shareUrl.indexOf('?') != -1 ? '&' : '?') + 'promotionId=' + this.promotionId;
        }
        if(this.couponCfgId){
            this.shareUrl = this.shareUrl + (this.shareUrl.indexOf('?') != -1 ? '&' : '?') + 'couponCfgId=' + this.couponCfgId;
        }
        if(this.tagNo){
            this.shareUrl = this.shareUrl + (this.shareUrl.indexOf('?') != -1 ? '&' : '?') + 'tagNo=' + this.tagNo;
        }
        if(this.queryWord){
            this.shareUrl = this.shareUrl + (this.shareUrl.indexOf('?') != -1 ? '&' : '?') + 'queryWord=' + this.queryWord;
        }
        if (this.distributorId || this.getStor('isDistributorId') != null) {
            this.shareUrl = this.shareUrl + (this.shareUrl.indexOf('?') != -1 ? '&' : '?') + "shareDistributorId=" + (this.getStor('isDistributorId') != null ? this.getStor('isDistributorId') : this.distributorId);
        }
        this.initList("add");
        // #ifdef H5
        this.$nextTick(() => {
    ...
    methods: {
        ...
        addCart(){
			...
			if(item.sellPrice != undefined && item.sellPrice != null && item.sellPrice != ''){
                ...
                }else if(this.getStor('isDistributorId')){
				...
				this.ebigRequest('/shopping-cart/add', obj, true).then((data) => {
                    ...
                    this.setStor('totalItemCount', this.cartQty)
                    uni.showToast({
                        title: "已加入清单",
                        icon: "none",
                    });
		...
		orderNeed(){
            ...
            if(this.distributorId){
                nowaddcartobj.distributorId = this.distributorId;
            }else if(this.getStor('isDistributorId')){
                nowaddcartobj.distributorId = this.getStor('isDistributorId');
            }
