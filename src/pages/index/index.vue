...
<script>
    ...
    export default {
        ...
        onLoad(option) {
            ...
            this._token = this.getStor("_token");
            ...
            this.$nextTick(() => {
                ...
                if(!this.urlStoreId){
                    let initStoreIdFunc = setInterval(() => {
                        let _storeId_ = this.getStor("_defaultStoreId_");
                        if (_storeId_ != undefined && _storeId_ != null && _storeId_ != '') {
                            ...
                            // #endif
                            this.$nextTick(() => {
                                clearInterval(initStoreIdFunc);
                            })
                        }
                    }, 100)
                }else{
                    this.setStor('_getLocation_', false);
                    this.pageUrl = pageUrl + '?storeId=' + this.storeId + '&consignorId=' + this.getStor('_defaultConsignorId_') + '&isiphoneBt=' + this.isiphoneBt + (_mobile_ ? '&_mobile_=' + _mobile_ : '') + '&isLogin=' + (isLogin == 'true' || isLogin == true ? true : false) + (this.getStor("isDistributorId") ? "&isDistributorId=" + this.getStor("isDistributorId") : '') + (this._token ? '&_token=' + this._token : '');
                    this.$nextTick(() => {
