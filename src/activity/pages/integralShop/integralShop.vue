    ...
    <div class="integralShop">
        ...
        <div class="goods_wrap">
            <div class="goods_wrap2 bg_white uni-flex">
                <div class="good_wrap" v-for="(item,index) in goodsList" :key="index" @click="goUrl('detail', '', 'redeemId=' + item.redeemId)">
                    <div class="img_wrap">
                    ...
                    <div class="uni-flex chang_warp">
                        ...
                        <div class="uni-flex-1 change_btn font_color_white" @click.stop="exchangeFunc(item.redeemId,item.redeemQty)">立即兑换</div>
                    </div>
                </div>
                <div class="nomore uni-flex-1" v-if="isBottom">没有更多数据</div>
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    methods:{
        ...
        getUserPoint(){
            var obj = {};
			obj.consignorId = this.getStor('_defaultConsignorId_');
            this.doRequest("/usercenter/memberpoints/amount", obj,false).then((data) => {
