...
export default {
    ...
    onLoad(option) {
        ...
        if(option.addrId != undefined && option.addrId != null){
            this.isEdit = true;
            this.doRequest("/usercenter/memberaddress/detail", {addressId: option.addrId}, true).then((response) => {
    ...
    methods: {
        ...
        setAddrObj(data){
            ...
            if(!this.isEdit){
                ...
                this.doRequest("/usercenter/area/convert", pcd, true, 'post', '', true).then((response) => {
                    if(response != null){
                        this.provinceId = response.provinceId;
                        this.cityId = response.cityId;
                        this.disrictId = response.disrictId;
        ...
        saveAddress(){
            ...
            uni.request({
                ...
                success: item => {
                    ...
                    this.doRequest("/usercenter/memberaddress/update", saveobj, true).then((res) => {
                        uni.showToast({
                            title: "地址保存成功！",
                            icon: "none",
                        });
