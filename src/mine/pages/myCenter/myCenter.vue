      ...
      >
        <view class="center_other_wrap clearfix">
          ...
          <view
            ...
            v-on:click="goMini()"
          >
          <i class="iconfont icon-myshare" style="color: #75b727"></i>
            <span>集团CRM</span>
          </view>
          <view
            href="javascript:void(0);"
  ...
  methods: {
    ...
    goMini() {
      uni.navigateToMiniProgram({
        appId: "wx20b43acadceabbd2",
        path: "/pages/index/index?mid=21e92247-0846-e911-8da7-005056aec674",
        envVersion: "release",
        success: (res) => {
          // 打开成功
        },
    ...
    initCode(type) {
      this.doRequest(
        type ? "/usercenter/invitecode/create" : "/usercenter/invitecode/mine",
        {
          consignorId: this.getStor("_defaultConsignorId_"),
        },
        true
    ...
    loginOut() {
      uni.showModal({
        ...
        success: (res1) => {
          if (res1.confirm) {
            this.doRequest(
              "/usercenter/member/login/invalidate",
              {},
              true
            ).then((res) => {
