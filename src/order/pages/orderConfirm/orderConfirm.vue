<template>
    <view class="orderConfirm">
        <!-- #ifndef MP-WEIXIN -->
        <uni-nav-hfbar fixed status-bar left-icon="back" title="需求提交"></uni-nav-hfbar>
        <!-- #endif -->
        <!-- 地址开始 -->
        <view class="address bg_white" @click="openAddress" v-if="deliveryType != '2'">
            <view class="addrCont uni-flex">
                <view class="uni-flex-1" v-if="orderAddress != null && orderAddress != ''">
                    <view class="userInfo">{{orderAddress.provinceName + orderAddress.cityName + (orderAddress.disrictName ? orderAddress.disrictName : '')}}</view>
                    <view class="addrInfo">{{orderAddress.address}}</view>
                    <view class="userInfo">{{orderAddress.linkManName + ' ' + orderAddress.mobile}}</view>
                </view>
        ...
        <!-- 门店信息结束 -->
        <view class="pageLine sendType bg_white uni-flex" style="border-top: 1px solid #F6F6F6;" :style="deliveryType == '1' && orderAddress != '' && orderAddress.supportDeliveryTypes && orderAddress.supportDeliveryTypes.indexOf(deliveryType) == -1 ? 'padding-bottom: 4rpx;' : ''" >
            <view class="plTip">配送方式</view>
            <view class=" font_color_common plCont uni-flex-1" :class="deliveryType == '1' && orderAddress != '' && orderAddress.supportDeliveryTypes && orderAddress.supportDeliveryTypes.indexOf(deliveryType) == -1 ? 'font_color_999' : ''">
                已选择：{{deliveryType == '1' ? '就近急送' : deliveryType == '2' ? '门店自提' : '快递配送'}}
            </view>
        </view>
            <view class="pageLine sendType bg_white uni-flex" style="border-top: 1px solid #F6F6F6;padding:0 50rpx" :style="deliveryType == '1' && orderAddress != '' && orderAddress.supportDeliveryTypes && orderAddress.supportDeliveryTypes.indexOf(deliveryType) == -1 ? 'padding-bottom: 4rpx;' : ''" >
            <scroll-view scroll-y="true" class="ppCont" style="min-height: 88rpx;">
            <radio-group @change="radioChange" v-if="storeDTO.deliverModes">
                <label class="sendTypeItem uni-flex" v-for="deliver in storeDTO.deliverModes" v-if="deliver != ''" :key="deliver">
                    <view class="uni-flex-1" :class="deliver == '1' && orderAddress != '' && orderAddress.supportDeliveryTypes && orderAddress.supportDeliveryTypes.indexOf('1') == -1 ? 'font_color_999' : ''">{{deliver == '1' ? ('就近急送' + (orderAddress != '' && orderAddress.supportDeliveryTypes && orderAddress.supportDeliveryTypes.indexOf('1') == -1 ? '(地址超出配送范围)' : '')) : deliver == '2' ? '到店自提' : '快递配送'}}</view>
                    <view>
            ...
            <view class="orderItem" v-for="od in orderDTO">
                ...
                <view class="orderGoods">
                    ...
                    <view>
                        <view class="goodsItem uni-flex" v-for="item in od.orderDtls">
                            ...
                            <view class="goodsDetails uni-flex-1">
                                ...
                                <view class="goodsSku wrap">{{item.specification}}</view>
                                <view class="goodsPriQyt uni-flex">
                                    <view class="goodsPri uni-flex-1 font_color_common" v-if="isRedeem">
                                        {{item.sellPrice.redeemPoint}}积分
                                    </view>
                                    <view class="goodsPri uni-flex-1 font_color_common" v-else-if="item.sellPrice != null">
                                        ￥{{item.sellPrice | initPrice}}
                                        <i v-if="item.sellPrice.promoPrice != undefined && item.sellPrice.promoPrice != null && item.sellPrice.promoPrice != '' && item.sellPrice.promoPrice != '0'">￥{{item.sellPrice.sellPrice}}</i>
                                    </view>
                                    <view class="goodsPri uni-flex-1 font_color_common" v-else>暂无报价</view>
                    ...
                    <view v-if="od.couponPromotioinDTOList != null && od.couponPromotioinDTOList.length > 0">
                        <view v-for="(item1,index1) in od.couponPromotioinDTOList" v-if="item1.canUse == true && item1.canGet == true && item1.check == true">
                ...
                <view class="font_color_common" style="padding: 0 30rpx 22rpx;" v-if="deliveryType == '1' && orderAddress != '' && orderAddress.supportDeliveryTypes && orderAddress.supportDeliveryTypes.indexOf(deliveryType) == -1">(您选择的配送地址超出就近急送范围，请重新选择)</view>
                ...
                <view class="pageLine sendType bg_white uni-flex" style="border-top: 1px solid #F6F6F6;" v-if="od.couponPromotioinDTOList != null && !isRedeem" @click="initCoupon(od.couponPromotioinDTOList, od.orderId);">
        ...
        <!-- 发票结束 -->
                <!-- 支付方式开始 -->
        <view class="pageLine payType bg_white uni-flex" v-if="isRedeem">
            <view class="plTip">支付方式</view>
            <view class="plCont uni-flex-1">
                积分兑换<view class="iconfont icon-right"></view>
            </view>
        </view>
        <view class="pageLine payType bg_white uni-flex" v-else>
            <view class="plTip">支付方式</view>
            <view class="plCont uni-flex-1">
        ...
        <view class="orderAmount bg_white" :style="isiphoneBt ? 'padding-bottom: 170rpx' : ''">
            <view class="oaItem uni-flex">
                商品金额<view class="oaItemCont uni-flex-1 font_color_common">{{isRedeem ? totalNeedPointsAmount + '积分' : '￥' + (totalAmount ? parseFloat(totalAmount).toFixed(2) : '0.00')}}</view>
            </view>
            <view class="oaItem uni-flex">
                配送费<view class="oaItemCont uni-flex-1 font_color_common">+ ￥{{deliveryFeeAmount | tofix2}}</view>
            </view>
            <view class="oaItem uni-flex" v-if="recipeFee && !isRedeem">
                开方费<view class="oaItemCont uni-flex-1 font_color_common">+ ￥{{recipeFee | tofix2}}</view>
            </view>
            <view class="oaItem uni-flex" v-if="promoDeliveryFeeAmount && !isRedeem">
                配送费立减<view class="oaItemCont uni-flex-1 font_color_common">- ￥{{promoDeliveryFeeAmount | tofix2}}</view>
            </view>
            <view class="oaItem uni-flex" v-if="actReduceFilter(discountAmount, promoReduceAmount) && !isRedeem">
                活动立减<view class="oaItemCont uni-flex-1 font_color_common">- ￥{{actReduceFilter(discountAmount, promoReduceAmount)}}</view>
            </view>
            <view class="oaItem uni-flex" v-if="couponReduceAmount && !isRedeem">
                优惠券<view class="oaItemCont uni-flex-1 font_color_common">- ￥{{couponReduceAmount | tofix2}}</view>
            </view>
            <view class="oaItem uni-flex" v-if="deductionAmount && !isRedeem">
        ...
        <view class="pageFoot uni-flex bg_white" :style="isiphoneBt ? 'padding-bottom: 40rpx' : ''">
            <view class="totalConfirm uni-flex-1">
                <view class="uni-flex">总计：<b class="font_color_common uni-flex-1">¥{{totalPayAmount | tofix2}} <span>{{isRedeem ? ' + ' + totalNeedPointsAmount + '积分': ''}}</span></b></view>
                ...
                <view class="ppCont">
                    <scroll-view class="couponCanuse" scroll-y="true" v-show="selectCoupontype == 'istrue'">
                        <view class="couponList" v-if="couponList != ''">
                            <checkbox-group>
                                <view class="couponItem" v-for="couponitem in couponList">
                                    <label @click="selectCoupon(couponitem.canGet,couponitem.check,couponitem.couponNo,couponitem.orderId)">
                                        <view>
                                            <view class="couponItemMain bg_common">
                                            ...
                                            <view class="couponItemDetails">
                                                <view class="couponItemMainP c-i-d-cont">
                                                    <view class="c-i-d-contSpan bg_common">{{couponitem.couponContentType | coupontypeFunc}}</view>
                                        ...
                                        <view class="couponItemS">
                                            <checkbox :value="couponitem.couponId" :checked="couponitem.check" />
                                        </view>
                                    </label>
                                    <view class="cannotselectTips" v-if="!couponitem.canGet">该优惠券不能与已选优惠券同时使用哦！</view>
                                </view>
                            </checkbox-group>
                        </view>
                    ...
                    <scroll-view class="couponCannotuse" scroll-y="true" v-show="selectCoupontype == 'isfalse'">
                        <view class="couponList" v-if="cannotcouponList != ''">
                            <view class="couponItem noselect" v-for="(cannotcouponitem, index) in cannotcouponList">
                                <view class="couponItemMain" :class="cannotcouponitem.canGet ? 'bg_common' : ''">
                                ...
                                <view class="couponItemDetails">
                                    <view class="couponItemMainP c-i-d-cont">
                                        <view class="c-i-d-contSpan" :class="cannotcouponitem.canGet ? 'bg_common' : ''">{{cannotcouponitem.couponContentType | coupontypeFunc}}</view>
...
export default {
    ...
    onLoad(option) {
        ...
        this.isRedeem = option.isRedeem ? option.isRedeem : '';
        this.initOrder();
    },
    methods: {
        openAddress(){
            // 跳转收货地址
            this.goUrl('addressList', 'mine', 'storeId=' + this.storeDTO.storeId + (this.orderAddress != '' ? '?addressId=' + this.orderAddress.addressId : ''));
        },
        openAskInfo(){
            // 跳转问诊信息
            this.goUrl('askInfo', 'order', 'storeId=' + this.storeDTO.storeId);
        },
        initOrder(addrId, couponSelectNo, couponSelectFlag, orderId){
            ...
            this.ebigRequest("/order/preview", obj, true).then((res) => {
                ...
                this.totalNeedPointsAmount = res.totalNeedPointsAmount
        ...
        resetOrderFunc(recipeId) {
            var getorderobj = {};
            var addrId = this.orderAddress ? this.orderAddress.addressId : '';
			if (addrId != '' && addrId != null && addrId != undefined) {
				getorderobj.addressId = addrId;
			}
			if (recipeId != null && recipeId != 'null' && recipeId != '') {
				getorderobj.recipeId = recipeId * 1;
			}
        ...
        zpTipFilter(order){  //判断是否显示活动提示
            ...
            if(order.couponPromotioinDTOList != undefined && order.couponPromotioinDTOList != null && order.couponPromotioinDTOList){
                for(var b=0;b<order.couponPromotioinDTOList.length;b++){
                    ...
                    if(item2.canUse == true && item2.canGet == true && item2.check == true){
        ...
        selectCoupon (canGet, check, couponno, orderId) { // 选择优惠券
			if (!canGet) {
                uni.showToast({
                    title: "不可与已选优惠券或促销活动叠加使用,已更换为当前选中优惠券!",
                    icon: "none",
                });
			}
            this.$nextTick(() => {
                this.initOrder('',couponno,!check, orderId);
            })
        },
        submitOrder(){
            // 提交订单
            if (this.orderAddress.addressId == '' && this.deliveryType != '2') {
                uni.showToast({
                    title: "抱歉！您还没选择收货地址",
                    icon: "none",
                });
			...
			if(this.deliveryType == '1' && this.orderAddress.supportDeliveryTypes && this.orderAddress.supportDeliveryTypes.indexOf('1') == -1){
                uni.showToast({
			...
			if (this.deliveryType != '2') {
				obj.addressId = this.orderAddress.addressId;
			}else if(this.takeDT != ''){
            ...
            let askObj = {
                ...
                drugUserId: this.druguser.userId,
                diseaseInfos: this.diseaseInfos,
                picInfos: this.historyImageArr,
                description: this.druguser.diseases
            }
            if(this.recipeToggle == 2){
                this.doRequest("/recipecenter/lianou/inquiry", askObj, false, 'json').then((res) => {
                    obj.recipeId = res;
                                ...
                                if (res) {
                                    ...
                                    this.ebigRequest("/order/getTrade", getTradeObj, false).then((response) => {
                                        ...
                                        if (this.isOrderNeed == true || this.isOrderNeed == 'true') {
                                            ...
                                            // #ifdef H5
                                            let pagePath = this.getWecharCodeUrl(this.h5APPID, true);
