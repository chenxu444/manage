<template>
  <view class="page">
    <!-- #ifndef MP-WEIXIN -->
    <uni-nav-hfbar fixed status-bar left-icon="back" rightText="全部订单" @click-right="toggleNav(0)" title="订单列表"></uni-nav-hfbar>
    <!-- #endif -->
    <!-- 头部状态导航 -->
      ...
      >
        <view>
          ...
          >
            ...
            >
              <img
                :src="goodsItem.defaultPic ? goodsItem.defaultPic : '../../../static/image/default/no_img.jpg'"
                class="goods_img"
                alt="暂无图片"
              />
              ...
              <view>
                <view class="price">
                  ...
                  <span v-else-if="goodsItem.itemType == '3'">
                    <span v-if="goodsItem.sellPrice && goodsItem.sellPrice.sellPrice">
                      {{
                        goodsItem.sellPrice.redeemPoint +
                        "积分 + " +
                        goodsItem.sellPrice.sellPrice +
                        "元"
                      }}
                    </span>
                    <span v-else>{{ goodsItem.sellPrice.redeemPoint + "积分" }}</span>
                  </span>
                  <span v-else
                    >￥{{
                      parseFloat(
            ...
            <view class="pay">
              ...
              <span
                ...
                }} {{item.orderType == 2 ? '+ ' + item.totalPointsAmount + '积分' : ''}}（含配送费￥{{
            ...
            >
              ...
              <button
                ...
                v-if="(item.orderStatus == '1' || item.orderStatus == '2') && item.orderType != 2"
              ...
              <button class="btns" @click="addAgain(item)" v-if="item.orderType != 2">再次购买</button>
              <!-- <button
            class="btns"
            v-if="
              item.orderStatus == '4' &&
...
export default {
  ...
  methods: {
    ...
    addAgain(row) {
      ...
      if(this.getStor('shareDistributorId')){
        condition.distributorId = this.getStor('shareDistributorId');
      }
